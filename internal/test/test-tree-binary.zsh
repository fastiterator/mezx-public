...
STR_G   ${selfS}_SE='
    if [[ ${okB} -ne ${B_T} ]]; then  ;
        ...
        eval ${evalS};  evalS=;  break;  fi;
    eval "${modUS}_V_replyI=\${replyI}  ${modUS}_V_replyS=\${replyS}";
    return ${B_T};
'
alias ${selfS}=${(P)${:-${selfS}_SE}}
...
function ${selfS}() {
    ...
    repeat 1; do
        ...
        calleemodUS=INTERNAL_TREE_BINARY calleeS=F_create curerrI=4;  TITB_CALLIT  ${modUS}_nodeSD  ${modUS}_F_comparator  0
        treerefS=${INTERNAL_TREE_BINARY_V_replyS}
        STR_G   ${modUS}_V_treerefS=${treerefS}
        okB=${B_T};  done
...
function ${selfS}() {
    ...
    repeat 1; do
        ...
        argcReqI=0;  eval ${modUS}_A_checkargc
        calleemodUS=INTERNAL_FRAMEWORK  calleeS=A_retsReset  curerrI=2;  TITB_CALLIT
        print ">>> enter non-negative numbers to insert, separated by [return] <<<"
        print "    enter \"s\" to search, \"d\" to delete, \"t\" to traverse, \"v\" to verbosely dump data structures, or \"q\" to quit"
        while read tmpS; do
            okB=${B_F}
            if [[ ${tmpS} =~ '^[pP].*$' ]]; then
                styleshortS='in'
                calleemodUS=INTERNAL_TREE_BINARY calleeS=F_traverse curerrI=$(( ${curerrI} + 1 ))
                print "*** calling: ${calleemodUS}_${calleeS} ${modUS}_nodeSD  ${(P)${:-${modUS}_V_treerefS}}  ${styleshortS}order  NONE  NONE ***"
                TITB_CALLIT  ${modUS}_nodeSD  \${${modUS}_V_treerefS}  ${styleshortS}order  NONE  NONE
                okB=${B_T};  continue;  fi
            if [[ ${tmpS} =~ '^[vV].*$' ]]; then
                styleshortS='in'
                eval "MAP_G   ${modUS}_V_dumpedM=( )"
                calleemodUS=${modUS} calleeS=F_mapDump curerrI=5;  TITB_CALLIT  \""${(P)${:-${modUS}_V_treerefS}}"\"  0  5
                okB=${B_T};  continue;  fi
            if [[ ${tmpS} =~ '^[sS].*$' ]]; then
                print ">>> enter number to search for <<<"
                read tmpS
                calleemodUS=INTERNAL_TREE_BINARY calleeS=F_search curerrI=$(( ${curerrI} + 3 ))
                print "*** calling: ${calleemodUS}_${calleeS} ${modUS}_nodeSD  ${(P)${:-${modUS}_V_treerefS}}  ${tmpS} ***"
                TITB_CALLIT  ${modUS}_nodeSD  \${${modUS}_V_treerefS}  \${tmpS}
                print -f "returned: errB=%d  replyB=%d  replyS=\"%s\"\n" -- \
                      ${(P)${:-${calleemodUS}_V_errB}}  ${(P)${:-${calleemodUS}_V_replyB}}  "${(P)${:-${calleemodUS}_V_replyS}}"
                okB=${B_T};  continue;  fi
            if [[ ${tmpS} =~ '^[tT].*$' ]]; then
                print ">>> enter number to search for (via traverse) <<<"
                read tmpS
                calleemodUS=INTERNAL_TREE_BINARY calleeS=F_search_via_traverse curerrI=$(( ${curerrI} + 3 ))
                print "*** calling: ${calleemodUS}_${calleeS} ${modUS}_nodeSD  ${(P)${:-${modUS}_V_treerefS}}  ${tmpS} ***"
                TITB_CALLIT  ${modUS}_nodeSD  \${${modUS}_V_treerefS}  \${tmpS}
                print -f "returned: errB=%d  replyB=%d  replyS=\"%s\"\n" -- \
                      ${(P)${:-${calleemodUS}_V_errB}}  ${(P)${:-${calleemodUS}_V_replyB}}  "${(P)${:-${calleemodUS}_V_replyS}}"
            ...
            if [[ ${tmpS} =~ '^[Dd].*$' ]]; then
                ...
                read tmpS
                calleemodUS=INTERNAL_TREE_BINARY calleeS=F_delete curerrI=$(( ${curerrI} + 4 ))
                print "*** calling: ${calleemodUS}_${calleeS} ${modUS}_nodeSD  ${(P)${:-${modUS}_V_treerefS}}  ${tmpS} ***"
                TITB_CALLIT  ${modUS}_nodeSD  \${${modUS}_V_treerefS}  \${tmpS}
                print -f "returned: replyB=%d\n" -- \
                      ${(P)${:-${calleemodUS}_V_replyB}}
                okB=${B_T};  continue;  fi
            if [[ ${tmpS} =~ '^[Qq].*$' ]]; then
                print "*** quitting... ***"
                okB=${B_T};  break;  fi
            if ! [[ ${tmpS} =~ ${INTERNAL_FRAMEWORK_C_reNumNonnegA_S} ]]; then
                print -f "ERROR: entry not in  ( p[rint]  v[erbose]  s[earch] t[raverse] d[elete] ); not a non-negative integer: \"%s\"\n" -- ${tmpS}
                okB=${B_T};  continue;  fi
            numI=${tmpS}
            okB=${B_F}
            calleemodUS=INTERNAL_TREE_BINARY calleeS=F_insert  curerrI=$(( ${curerrI} + 3 ))
            TITB_CALLIT  ${modUS}_nodeSD  \${${modUS}_V_treerefS}  \${numI}
            print -f "returned: errB=%d  replyB=%d  replyS=\"%s\"\n" -- \
                  ${(P)${:-${calleemodUS}_V_errB}}  ${(P)${:-${calleemodUS}_V_replyB}}  "${(P)${:-${calleemodUS}_V_replyS}}"
            okB=${B_T};  done
        if [[ ${okB} -ne ${B_T} ]]; then
	    if [[ ${(P)+${:-${calleemodUS}_errB}} -eq 1 ]]; then
		if [[ ${(P)${:-${calleemodUS}_errB}} -eq ${B_T} ]]; then
		    errI=$(( ${curerrI} + 4 ))
		    print -v errS -f "call to \"%s\" failed:  { %s;  errI=%d }" -- \
			  ${calleemodUS}_${calleeS}  ${(P)${:-${calleemodUS}_errS}}  ${(P)${:-${calleemodUS}_errI}}
		    break;  fi
		errI=$(( ${curerrI} + 5 ))
		print -v errS -f "unknown error; called \"%s\"; var \"%s\" does not exist" -- \
		      ${calleemodUS}_${calleeS}  ${calleemodUS}_errB;  break;  fi
	    break;  fi
        okB=${B_T};  done
    eval ${modUS}_A_funcEnd
}
...
function ${selfS}() {
    ...
    repeat 1; do
        ...
        for  iI in {1..2}; do
            eval "valueS=\$${iI}"
            if ! [[ ${valueS} =~ ${INTERNAL_FRAMEWORK_C_reNumNonnegA_S} ]]; then
                errI=$(( 1 + ${iI} ))
                print -v errS -f "value of \"%s%s%s\" arg does not match \"%s\" regex (\"%s\"): \"%s\"" -- \
                      value  ${iI}  _S  INTERNAL_FRAMEWORK_C_reNumNonnegA_S  ${INTERNAL_FRAMEWORK_C_reNumNonnegA_S}  ${valueS};  break 2;  fi
            eval "value${iI}_S=\${valueS}";  done
        if [[ ${value1_S} -eq ${value2_S} ]]; then  replyI=${INTERNAL_TREE_BINARY_C_compareEqualI}
        elif [[ ${value1_S} -lt ${value2_S} ]]; then  replyI=${INTERNAL_TREE_BINARY_C_compareOrderedI}
...
if [[ ${(P)${:-${modUS}_V_errB}} -ne ${B_F} ]]; then
    ...
    print -f "var \"%s\"=\"%s\"\n" --  ${modUS}_V_treerefS  ${(P)${:-${modUS}_V_treerefS}}
    calleemodUS=${modUS}
    calleeS=F_mapDump curerrI=5;  TITB_CALLIT  \${${modUS}_V_treerefS}  0  20
    exit 1;  fi
exit 0
