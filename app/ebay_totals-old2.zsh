...
LIST_L  modL=( app/ebay_csv_parse  app/ebay_marketing_csv_parse  app/mrg_csv_parse  app/pirate_csv_parse );  INTERNAL_FRAMEWORK_A_loadDependencies
print '  DONE'
...
for idS in ${(k)APP_EBAY_CSV_PARSE_V_orderM}; do
    ...
    itemS_feeF_L=()
    fee_subtotal_F=0
    if [[ ! -v "APP_MRG_CSV_PARSE_V_orderM[${idS}]" ]]; then
	print -f 'WARNING: %s: order-id not found in mrg data: "%s"\n' -- \
	      $0  ${idS//_/-}
    else
	mcountI=${APP_MRG_CSV_PARSE_V_orderM[${idS}]}
	for numI in {1..${mcountI}}; do
	    eval MAP_L infoM="( \"\${(kv@)APP_MRG_CSV_PARSE_V_order_${idS}_${numI}_M}\" )"
	    feeF=${infoM[transaction-fees]/$/}
	    itemS_feeF_L+=( ${feeF} )
