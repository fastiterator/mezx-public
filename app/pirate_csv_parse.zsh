...
STR_L   modS=app/pirate_csv_parse
...
function ${selfS}() {
	...
	headerS=${headerS/\/}
	requiredfieldL=( source  order-id  cost )
	...
	tail +2 ${pathnameS} | while read -r lineS; do
	    ...
	    source_S=${LIB_CSV_V_replyM[source]}
	    order_id_S=${LIB_CSV_V_replyM[order-id]}
	    cost_S=${LIB_CSV_V_replyM[cost]}
	    order_id_S=${order_id_S//-/_}
	    LIB_CSV_V_replyM[order-id]=${order_id_S}
	    if [[ ${order_id_S} = '' ]]; then
		order_id_S=${LIB_CSV_V_replyM[tracking-number]};  fi
	    varnameS="${modUS}_V_orderM[\${order_id_S}]"
	    if ! [[ -v ${varnameS} ]]; then
		eval "${varnameS}=0"; fi
	    eval "${varnameS}=\$(( \${${varnameS}} + 1 ));  subnumI=\${${varnameS}}"
	    varnameS="${modUS}_V_order_${order_id_S}_${subnumI}_M"
	    eval "MAP_G   ${varnameS}=()"
