...
STR_L     idS  promoteS  tmpS  titleS
...
for idS in ${(k)APP_EBAY_CSV_PARSE_V_orderM}; do
    ...
    print -f 'order-number=%s  sale-date=%s  sold-for=%.2f  s-and-h=%.2f  pirate-cost=%.2f  fee-subtotal=%.2f  promote-subtotal=%.2f  items=( %s )  items-fees=( %s )  items-promote=( %s )  item-title=%q\n' -- \
