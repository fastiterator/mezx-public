...
STR_L   modS=app/ebay_marketing_csv_parse
STR_L   modUS=${(U)modS//\//_}
MAP_G   ${modUS}_V_campaignS_itemS_M
MAP_G   ${modUS}_V_campaignS_M
MAP_G   ${modUS}_V_itemS_M
...
#
function ${selfS}() {
    eval APP_EBAY_MARKETING_CSV_PARSE_A_selfmodSetup
    eval INTERNAL_FRAMEWORK_A_funcStart
    STR_L   pathnameS  headerS  lineS  varnameS
    STR_L   campaign_id_S  item_id_S
    INT_L   subnumI
	...
	headerS=${headerS/\/}
	requiredfieldL=( campaign-id  item-id )
	...
	tail +2 ${pathnameS} | while read -r lineS; do
	    ...
	    campaign_id_S=${LIB_CSV_V_replyM[campaign-id]}
	    varnameS="${modUS}_V_campaignS_M[${campaign_id_S}]"
	    if [[ ! -v ${varnameS} ]]; then
		eval "${varnameS}=0"; fi
	    eval "${varnameS}=\$(( \${$varnameS} + 1 ))"
	    item_id_S=${LIB_CSV_V_replyM[item-id]}
	    varnameS="${modUS}_V_itemS_M[${item_id_S}]"
	    if [[ ! -v ${varnameS} ]]; then
		eval "${varnameS}=0"; fi
	    eval "${varnameS}=\$(( \${$varnameS} + 1 ))"
	    varnameS="${modUS}_V_campaignS_itemS_M[\${campaign_id_S}-${item_id_S}]"
	    if ! [[ -v ${varnameS} ]]; then
		eval "${varnameS}=0"; fi
	    eval "${varnameS}=\$(( \${${varnameS}} + 1 ));  subnumI=\${${varnameS}}"
	    varnameS="${modUS}_V_campaign_${campaign_id_S}_item_${item_id_S}_${subnumI}_M"
	    eval "MAP_G   ${varnameS}=()"
